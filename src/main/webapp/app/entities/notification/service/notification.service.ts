...
export class NotificationService {
  ...
  protected convertDateFromClient(notification: INotification): INotification {
    return Object.assign({}, notification, {
      date: notification.date?.isValid() ? notification.date.format(DATE_FORMAT) : undefined,
      dateProchaineMaintenance: notification.dateProchaineMaintenance?.isValid()
        ? notification.dateProchaineMaintenance.format(DATE_FORMAT)
        : undefined,
    });
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.dateProchaineMaintenance = res.body.dateProchaineMaintenance ? dayjs(res.body.dateProchaineMaintenance) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((notification: INotification) => {
        ...
        notification.dateProchaineMaintenance = notification.dateProchaineMaintenance
          ? dayjs(notification.dateProchaineMaintenance)
          : undefined;
      });
    }
    return res;
