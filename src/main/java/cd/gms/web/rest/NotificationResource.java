...
import cd.gms.service.NotificationService;
...
/**
 * REST controller for managing {@link cd.gms.domain.Notification}.
 */
@RestController
@RequestMapping("/api")
public class NotificationResource {
    ...
    public ResponseEntity<List<Notification>> getAllNotifications(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("maintenance-is-null".equals(filter)) {
            log.debug("REST request to get all Notifications where maintenance is null");
            return new ResponseEntity<>(notificationService.findAllWhereMaintenanceIsNull(), HttpStatus.OK);
        }
